 ...
 *
 * @param sector the sector where the directory is to be created
 * @param entry_cnt the number of entries the directory should have
 * @param parent_sector the sector of the parent directory
...
bool dir_create (block_sector_t sector, size_t entry_cnt, block_sector_t parent_sector)
{
  ...
  struct dir_entry cur_prev[2] = {
      {sector, ".", true},
      {parent_sector, "..", true}
  };

  if (!inode_write_at (dir->inode, cur_prev, sizeof (cur_prev), 0))
    success = false;
...
/** Opens and returns the directory for the given INODE, of which
   ...
   Must make sure that inode is a directory!
...
/** According to a path, opens a directory and returns it if find it,
...
dir_open_path (const char *path)
{
  ...
  /* Look up directory. */
  char *token, *saved_p;
  for (token = strtok_r (path_copy, "/", &saved_p); token != NULL;
       token = strtok_r (NULL, "/", &saved_p))
    {
      struct inode *inode = NULL;
      if (!dir_lookup (dir, token, &inode))
        {
          // Cannot find file with the name.
          dir_close (dir);
          return NULL;
        }
...
dir_remove (struct dir *dir, const char *name)
{
  ...
  if (inode_is_dir (inode))
    {
      /* If directory's inode is opened, cannot remove. */
      if (inode_get_open_cnt (inode) > 1)
        goto done;

      struct dir *dir = dir_open (inode);
      /* If try to remove cwd, cannot remove it. */
      if (inode_get_inumber (dir_get_inode (thread_current ()->cwd)) == inode_get_inumber (inode))
        {
          dir_close (dir);
          goto done;
        }
      /* If dir is not empty, cannot remove it. */
      if (!dir_is_empty (dir))
        {
          dir_close (dir);
          goto done;
...
{
  struct dir_entry e;
  off_t ofs = 2 * sizeof (struct dir_entry);  // Jump the first two entries("." and "..")
